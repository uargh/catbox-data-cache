...
function WXS(properties) {
  this.wxs = properties;
  if (this.wxs.restResourceSecure.indexOf('https://') != 0)
    this.wxs.restResourceSecure = 'https://' + this.wxs.restResourceSecure;
  this.auth = 'Basic '
      + new Buffer(this.wxs.username + ':' + this.wxs.password)
          .toString('base64');
  var p = url.parse(this.wxs.restResourceSecure);
  this.baseUrl = 'https://' + p.hostname;
